<div align="center">

# Hi, I'm Salim Loucif 👋

### AI Engineer · Software Engineer · Product Builder

I design and build production-grade AI systems, intelligent automation platforms, and scalable software products.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Salim%20Loucif-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/salimloudev/)
[![Portfolio](https://img.shields.io/badge/Portfolio-salimloudev.com-111111?style=for-the-badge&logo=vercel&logoColor=white)](https://salimloucif.com/)
[![SefarAI](https://img.shields.io/badge/SefarAI-AI%20Solutions-6C63FF?style=for-the-badge&logo=openai&logoColor=white)](https://sefarai.com/)
[![GitHub](https://img.shields.io/badge/GitHub-salimloudev-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/salimloudev)

![Profile Views](https://komarev.com/ghpvc/?username=salimloudev&style=flat-square&label=Profile+Views)

</div>

---

## About Me

I am an **AI and Software Engineer based in Dubai, UAE**, with an MSc in Artificial Intelligence and more than four years of experience building intelligent software and data-driven products.

My work focuses on transforming complex business processes into reliable, secure, and scalable systems using:

- Agentic AI and multi-agent orchestration
- Large language models and retrieval-augmented generation
- Machine learning and predictive analytics
- Document AI, OCR, and automated validation
- Real-time voice and conversational AI
- Backend platforms, APIs, and workflow automation

I enjoy taking AI products from early prototypes to production-ready systems while aligning technical decisions with measurable business outcomes.

---

## What I Build

### Intelligent AI Systems

Production AI applications using LLMs, RAG, tool calling, structured outputs, evaluation pipelines, and multi-agent workflows.

### Business Automation

Systems that automate document processing, decision-making, data validation, customer support, and operational workflows.

### Scalable Software Platforms

Reliable backend services, APIs, data pipelines, observability systems, and cloud-native applications built for real-world usage.

---

## Selected Work

### Multi-Agent AI Orchestration Platform

Designed an orchestration layer where specialized AI agents collaborate, exchange context, execute tools, validate outputs, and complete multi-step workflows.

`LangGraph` `Python` `FastAPI` `LLMs` `RAG` `Tool Calling`

### Enterprise Knowledge and RAG Systems

Built retrieval systems that connect language models with private organizational data while supporting semantic search, citations, reranking, access control, and response validation.

`Qdrant` `LlamaIndex` `LangChain` `Embeddings` `Reranking`

### Document AI and Automated Validation

Developed workflows for extracting, validating, and processing information from business documents using OCR, machine learning, APIs, and configurable business rules.

`OCR` `Computer Vision` `Python` `.NET` `SQL Server`

### Voice and Conversational AI

Created real-time voice-agent prototypes capable of understanding users, accessing business tools, retrieving information, and responding naturally.

`LiveKit` `ElevenLabs` `FastAPI` `LLMs`

### Machine-Learning Forecasting Systems

Developed predictive models and data pipelines for valuation, forecasting, anomaly detection, and automated decision support.

`Python` `Pandas` `Scikit-learn` `TensorFlow` `SQL`

---

## Products and Projects

<table>
  <tr>
    <td width="50%">
      <h3 align="center">ContentFlow</h3>
      <p align="center">
        An AI-powered content creation and workflow automation platform.
      </p>
      <p align="center">
        <a href="https://contentflow.sefarai.com/"><strong>Explore ContentFlow →</strong></a>
      </p>
    </td>
    <td width="50%">
      <h3 align="center">Waqto Salat</h3>
      <p align="center">
        A prayer-time application with configurable alarms, daily schedules, and Qibla direction.
      </p>
      <p align="center">
        <a href="https://waqtosalat.com/"><strong>Visit Waqto Salat →</strong></a>
      </p>
    </td>
  </tr>
  <tr>
    <td width="50%">
      <h3 align="center">SefarAI</h3>
      <p align="center">
        An AI software solutions studio building secure autonomous systems and intelligent automation.
      </p>
      <p align="center">
        <a href="https://sefarai.com/"><strong>Visit SefarAI →</strong></a>
      </p>
    </td>
    <td width="50%">
      <h3 align="center">Developer Portfolio</h3>
      <p align="center">
        A collection of my professional work, technical experience, and selected projects.
      </p>
      <p align="center">
        <a href="https://portfolio.salimloudev.com/"><strong>View Portfolio →</strong></a>
      </p>
    </td>
  </tr>
</table>

---

## Technology Stack

### AI, LLMs and Machine Learning

<p>
  <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=flat-square&logo=langchain&logoColor=white" alt="LangChain" />
  <img src="https://img.shields.io/badge/LangGraph-1C3C3C?style=flat-square&logo=langchain&logoColor=white" alt="LangGraph" />
  <img src="https://img.shields.io/badge/LlamaIndex-6B46C1?style=flat-square" alt="LlamaIndex" />
  <img src="https://img.shields.io/badge/RAG-005571?style=flat-square" alt="RAG" />
  <img src="https://img.shields.io/badge/Agentic_AI-111111?style=flat-square&logo=openai&logoColor=white" alt="Agentic AI" />
  <img src="https://img.shields.io/badge/Multi--Agent_Systems-6C63FF?style=flat-square" alt="Multi-Agent Systems" />
  <img src="https://img.shields.io/badge/Hugging_Face-FFD21E?style=flat-square&logo=huggingface&logoColor=black" alt="Hugging Face" />
  <img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=flat-square&logo=tensorflow&logoColor=white" alt="TensorFlow" />
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white" alt="PyTorch" />
  <img src="https://img.shields.io/badge/scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white" alt="Scikit-learn" />
  <img src="https://img.shields.io/badge/OpenCV-5C3EE8?style=flat-square&logo=opencv&logoColor=white" alt="OpenCV" />
</p>

### Languages and Backend Engineering

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" alt="Python" />
  <img src="https://img.shields.io/badge/C%23-512BD4?style=flat-square&logo=csharp&logoColor=white" alt="C Sharp" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" alt="JavaScript" />
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white" alt="FastAPI" />
  <img src="https://img.shields.io/badge/.NET-512BD4?style=flat-square&logo=dotnet&logoColor=white" alt=".NET" />
  <img src="https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white" alt="Flask" />
  <img src="https://img.shields.io/badge/Django-092E20?style=flat-square&logo=django&logoColor=white" alt="Django" />
  <img src="https://img.shields.io/badge/REST_APIs-005571?style=flat-square" alt="REST APIs" />
</p>

### Data, Infrastructure and Cloud

<p>
  <img src="https://img.shields.io/badge/SQL_Server-CC2927?style=flat-square&logo=microsoftsqlserver&logoColor=white" alt="SQL Server" />
  <img src="https://img.shields.io/badge/Qdrant-DC244C?style=flat-square&logo=qdrant&logoColor=white" alt="Qdrant" />
  <img src="https://img.shields.io/badge/Pandas-150458?style=flat-square&logo=pandas&logoColor=white" alt="Pandas" />
  <img src="https://img.shields.io/badge/NumPy-013243?style=flat-square&logo=numpy&logoColor=white" alt="NumPy" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white" alt="Docker" />
  <img src="https://img.shields.io/badge/Azure-0078D4?style=flat-square&logo=microsoftazure&logoColor=white" alt="Azure" />
  <img src="https://img.shields.io/badge/AWS-232F3E?style=flat-square&logo=amazonwebservices&logoColor=white" alt="AWS" />
  <img src="https://img.shields.io/badge/Google_Cloud-4285F4?style=flat-square&logo=googlecloud&logoColor=white" alt="Google Cloud" />
  <img src="https://img.shields.io/badge/Grafana-F46800?style=flat-square&logo=grafana&logoColor=white" alt="Grafana" />
  <img src="https://img.shields.io/badge/Prometheus-E6522C?style=flat-square&logo=prometheus&logoColor=white" alt="Prometheus" />
  <img src="https://img.shields.io/badge/Langfuse-111111?style=flat-square" alt="Langfuse" />
</p>

### Development Tools

<p>
  <img src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" alt="Git" />
  <img src="https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white" alt="GitHub" />
  <img src="https://img.shields.io/badge/Visual_Studio-5C2D91?style=flat-square&logo=visualstudio&logoColor=white" alt="Visual Studio" />
  <img src="https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white" alt="Visual Studio Code" />
  <img src="https://img.shields.io/badge/Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white" alt="Jupyter" />
  <img src="https://img.shields.io/badge/Selenium-43B02A?style=flat-square&logo=selenium&logoColor=white" alt="Selenium" />
</p>

---

## Current Focus

I am currently exploring and building with:

- Reliable agentic AI architectures
- LangGraph-based multi-agent workflows
- Model Context Protocol (MCP)
- Agent-to-Agent communication
- AI evaluation and observability
- Secure enterprise RAG systems
- Human-in-the-loop approval workflows
- Production guardrails and output validation

---

## Engineering Principles

```text
Build for production, not only for demonstrations.
Measure outcomes, not only model accuracy.
Keep humans in control of high-impact decisions.
Design AI systems that are observable, secure, and maintainable.
```

---

## GitHub Activity

<div align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=salimloudev&show_icons=true&include_all_commits=true&count_private=true&theme=github_dark&hide_border=true" alt="Salim's GitHub statistics" />

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=salimloudev&layout=compact&langs_count=8&theme=github_dark&hide_border=true" alt="Salim's most used languages" />

<img width="90%" src="https://github-readme-activity-graph.vercel.app/graph?username=salimloudev&theme=github-compact&hide_border=true&area=true" alt="Salim's contribution activity" />

</div>

---

## Let's Connect

I am always interested in discussing:

- Production AI and machine-learning systems
- Agentic and multi-agent architectures
- AI engineering opportunities
- Open-source collaboration
- Product development and technical partnerships

The best way to reach me is through [LinkedIn](https://www.linkedin.com/in/salimloudev/).

<div align="center">

### Building intelligent systems that solve real business problems.

[LinkedIn](https://www.linkedin.com/in/salimloudev/) ·
[Portfolio](https://portfolio.salimloudev.com/) ·
[SefarAI](https://sefarai.com/) ·
[GitHub](https://github.com/salimloudev)

</div>
